...
def train(epoch):
    ...
    for idx in range(x_train.size(0)):
        ...
        for i in range(1, 100):
            a = 2 * np.random.randint(0, 2, size=13) - 1
            r = min(l)
            x = np.random.uniform(0, radiu[r], size=13)
...
if __name__ == '__main__':
    ...
    #torch.save(net.state_dict(), 'nBnet_params_1.pkl')

    new_net = Net()
    new_net.eval()
    new_net.load_state_dict(torch.load('nBnet_params_6.pkl'))
