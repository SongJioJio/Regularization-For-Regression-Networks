...
def test():
    ...
    for idx in range(1, x0.size(0)+1):
        #x = x0[idx - 1].unsqueeze(dim=0)
        #output = new_net(x)
        output = new_net(x0[idx - 1])
        loss = loss_f(output, u[idx - 1])
        c = c+loss
        #loss = torch.abs(output - u[idx - 1])
        if idx % 1 == 0:
            with torch.no_grad():
                print('test Epoch:{} {} Loss {:.6f}'.format(1, idx, loss.item()))
...
if __name__ == '__main__':
    ...
    torch.save(net.state_dict(), 'nmnet_params_e2.pkl')
    #wandb.save('tnet_params_91.pkl')


    new_net = Net()
    new_net.load_state_dict(torch.load('nmnet_params_e2.pkl'))
